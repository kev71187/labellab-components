...
import { getRandomDocument } from '../../actions/annotations.js'
// import TwoClickBoundingBox from '../../components/labelers/TwoClickBoundingBox'
...
class Show extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    const {daId, datasetsDocument} = nextProps
  ...
  render() {
    const { doc, datasetsDocument, dataset, label, box, daId, ddId } = this.props
...
const mapStateToProps = (state, props) =>  {
  const {imageLabel: {box, question, rotation}} = state
  const {doc} = props
  return {box, question, rotation}
}
